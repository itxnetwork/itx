...
import (
	...
	cryptocodec "github.com/itxnetwork/itxitx/v12/crypto/codec"
	"github.com/itxnetwork/itxitx/v12/ethereum/eip712"
...
func PrepareEIP712CosmosTx(
	...
	accNumber := appItx.AccountKeeper.GetAccount(ctx, from).GetAccountNumber()

	nonce, err := appItx.AccountKeeper.GetSequence(ctx, from)
	if err != nil {
		return nil, err
	}
...
func createTypedData(args typedDataArgs, useLegacy bool) (apitypes.TypedData, error) {
	if useLegacy {
		...
		itxCodec := codec.NewProtoCodec(registry)
