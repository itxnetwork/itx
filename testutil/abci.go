...
import (
	...
	"github.com/itxnetwork/itx/v12/testutil/tx"
)
...
//  3. EndBlock
//  4. Commit
func Commit(ctx sdk.Context, app *app.Itx, t time.Duration, vs *tmtypes.ValidatorSet) (sdk.Context, error) {
...
func DeliverTx(
	...
	return BroadcastTxBytes(appItx, txConfig.TxEncoder(), tx)
}
...
func DeliverEthTx(
	...
	tx, err := tx.PrepareEthTx(txConfig, appItx, priv, msgs...)
	if err != nil {
		return abci.ResponseDeliverTx{}, err
	}
	return BroadcastTxBytes(appItx, txConfig.TxEncoder(), tx)
...
func CheckTx(
	...
	return checkTxBytes(appItx, txConfig.TxEncoder(), tx)
}
...
func BroadcastTxBytes(app *app.Itx, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseDeliverTx, error) {
...
func checkTxBytes(app *app.Itx, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseCheckTx, error) {
	bz, err := txEncoder(tx)
	if err != nil {
		return abci.ResponseCheckTx{}, err
	}
