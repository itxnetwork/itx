...
func DefaultConfig() Config {
	...
	return Config{
		...
		BondedTokens:      sdk.TokensFromConsensusPower(100000000000000000, itxtypes.PowerReduction),
...
// NewAppConstructor returns a new Itx AppConstructor
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	if !itxtypes.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "itxd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "itxcli")
		...
		customAppTemplate, _ := config.AppConfig(itxtypes.AttoItx)
