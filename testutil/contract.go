...
func DeployContract(
	...
	chainID := itxApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := itxApp.EvmKeeper.GetNonce(ctx, from)
	...
	msgEthereumTx := evm.NewTx(&evm.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: itxApp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := DeliverEthTx(itxApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, itxApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := itxApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := itxApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := itxApp.EvmKeeper.GetNonce(ctx, from)
