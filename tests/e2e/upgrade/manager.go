	...
	err = m.pool.Retry(
		func() error {
			...
			if c.State.ExitCode != 0 {
				stdOut, stdErr, _ := m.GetLogs(resource.Container.ID)
				return fmt.Errorf(
					"can't start itx node, container exit code: %d\n\n[error stream]:\n\n%s\n\n[output stream]:\n\n%s",
					c.State.ExitCode,
					stdErr,
					stdOut,
				)
...
// WaitForHeight queries the Itx node every second until the node will reach the specified height.
...
// GetNodeHeight calls the Itx CLI in the current node container to get the current block height
func (m *Manager) GetNodeHeight(ctx context.Context) (int, error) {
	exec, err := m.CreateExec([]string{"itxd", "q", "block"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return 0, fmt.Errorf("itx query error: %s", errBuff.String())
	}
	return h, nil
}

// GetNodeVersion calls the Itx CLI in the current node container to get the
// current node version
func (m *Manager) GetNodeVersion(ctx context.Context) (string, error) {
	exec, err := m.CreateExec([]string{"itxd", "version"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return "", fmt.Errorf("itx version error: %s", errBuff.String())
	}
