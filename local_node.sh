...
LOGLEVEL="info"
# Set dedicated home directory for the itxd instance
HOMEDIR="$HOME/.tmp-itxd"
# to trace evm
#TRACE="--trace"
TRACE=""
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	itxd config keyring-backend $KEYRING --home "$HOMEDIR"
	itxd config chain-id $CHAINID --home "$HOMEDIR"

	# If keys exist they should be deleted
	for KEY in "${KEYS[@]}"; do
		itxd keys add "$KEY" --keyring-backend $KEYRING --algo $KEYALGO --home "$HOMEDIR"
	done
	...
	claims_key=${KEYS[0]}
	node_address=$(itxd keys show "$claims_key" --keyring-backend $KEYRING --home "$HOMEDIR" | grep "address" | cut -c12-)
	...
	# itx15cvq3ljql6utxseh0zau9m8ve2j8erz8w5gvad
	jq -r --arg amount_to_claim "$amount_to_claim" '.app_state["bank"]["balances"] += [{"address":"itx15cvq3ljql6utxseh0zau9m8ve2j8erz8w5gvad","coins":[{"denom":"aitx", "amount":$amount_to_claim}]}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	## 3. Clone this ~/.itxd home directory into some others, let's say `~/.clonedItxd`
	## 4. Run `gentx` in each of those folders
	## 5. Copy the `gentx-*` folders under `~/.clonedItxd/config/gentx/` folders into the original `~/.itxd/config/gentx`

	# Collect genesis tx
	itxd collect-gentxs --home "$HOMEDIR"

	# Run this to ensure everything worked and that the genesis file is setup correctly
	itxd validate-genesis --home "$HOMEDIR"
