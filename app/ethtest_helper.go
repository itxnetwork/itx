...
// EthSetup initializes a new ItxApp. A Nop logger is set in ItxApp.
func EthSetup(isCheckTx bool, patchGenesis func(*Itx, simapp.GenesisState) simapp.GenesisState) *Itx {
	return EthSetupWithDB(isCheckTx, patchGenesis, dbm.NewMemDB())
}

// EthSetupWithDB initializes a new ItxApp. A Nop logger is set in ItxApp.
func EthSetupWithDB(isCheckTx bool, patchGenesis func(*Itx, simapp.GenesisState) simapp.GenesisState, db dbm.DB) *Itx {
	app := NewItx(log.NewNopLogger(),
		db,
		nil,
		true,
		map[int64]bool{},
