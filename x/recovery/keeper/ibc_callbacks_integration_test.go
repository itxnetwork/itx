...
var _ = Describe("Recovery: Performing an IBC Transfer", Ordered, func() {
	...
	Describe("from a non-authorized chain", func() {
		BeforeEach(func() {
			...
			err := s.ItxChain.App.(*app.Itx).ClaimsKeeper.SetParams(s.ItxChain.GetContext(), params)
	...
	Describe("from an authorized, non-EVM chain (e.g. Osmosis)", func() {
		Describe("to a different account on Itx (sender != recipient)", func() {
			BeforeEach(func() {
				sender = s.IBCOsmosisChain.SenderAccount.GetAddress().String()
				receiver = s.ItxChain.SenderAccount.GetAddress().String()
				senderAcc = sdk.MustAccAddressFromBech32(sender)
		...
		Describe("to the sender's own eth_secp256k1 account on Itx (sender == recipient)", func() {
			BeforeEach(func() {
				sender = s.IBCOsmosisChain.SenderAccount.GetAddress().String()
			...
			Context("with disabled recovery parameter", func() {
				BeforeEach(func() {
					...
					s.ItxChain.App.(*app.Itx).RecoveryKeeper.SetParams(s.ItxChain.GetContext(), params) //nolint:errcheck
				})
			...
			Context("with a sender's claims record", func() {
				...
				Context("with completed actions", func() {
					...
					It("should not claim/migrate/merge claims records", func() {
						...
						claimAfter, _ := s.ItxChain.App.(*app.Itx).ClaimsKeeper.GetClaimsRecord(s.ItxChain.GetContext(), senderAcc)
			...
			Context("without a sender's claims record", func() {
				...
				When("recipient has additional ibc vouchers that originated from other chains", func() {
					BeforeEach(func() {
						...
						// Send uatom from Cosmos to Itx
						s.SendAndReceiveMessage(s.pathCosmosItx, s.IBCCosmosChain, coinAtom.Denom, coinAtom.Amount.Int64(), s.IBCCosmosChain.SenderAccount.GetAddress().String(), receiver, 1)

						params.EnableRecovery = true
						s.ItxChain.App.(*app.Itx).RecoveryKeeper.SetParams(s.ItxChain.GetContext(), params) //nolint:errcheck
					})
					It("should not recover tokens that originated from other chains", func() {
						// Send uosmo from Osmosis to Itx
						s.SendAndReceiveMessage(s.pathOsmosisItx, s.IBCOsmosisChain, "uosmo", 10, sender, receiver, 1)
						...
						ibcAtom := s.ItxChain.App.(*app.Itx).BankKeeper.GetBalance(s.ItxChain.GetContext(), senderAcc, teststypes.UatomIbcdenom)
						Expect(ibcAtom).To(Equal(sdk.NewCoin(teststypes.UatomIbcdenom, coinAtom.Amount)))

						// Repeat transaction from Osmosis to Itx
						s.SendAndReceiveMessage(s.pathOsmosisItx, s.IBCOsmosisChain, "uosmo", 10, sender, receiver, 2)
				...
				When("recipient has additional non-native ibc vouchers that originated from senders chains", func() {
					BeforeEach(func() {
						params := types.DefaultParams()
						params.EnableRecovery = false
						err := s.ItxChain.App.(*app.Itx).RecoveryKeeper.SetParams(s.ItxChain.GetContext(), params)
						s.Require().NoError(err)
						...
						transferMsg := transfertypes.NewMsgTransfer(s.pathOsmosisItx.EndpointA.ChannelConfig.PortID, s.pathOsmosisItx.EndpointA.ChannelID, sdk.NewCoin(teststypes.UatomIbcdenom, sdk.NewInt(10)), sender, receiver, timeoutHeight, 0, "")
						...
						packet := channeltypes.NewPacket(transfer.GetBytes(), 1, s.pathOsmosisItx.EndpointA.ChannelConfig.PortID, s.pathOsmosisItx.EndpointA.ChannelID, s.pathOsmosisItx.EndpointB.ChannelConfig.PortID, s.pathOsmosisItx.EndpointB.ChannelID, timeoutHeight, 0)
						// Receive message on the itx side, and send ack
						err = s.pathOsmosisItx.RelayPacket(packet)
						s.Require().NoError(err)

						// Check that the ibc/uatom are available
						osmoIBCAtom := s.ItxChain.App.(*app.Itx).BankKeeper.GetBalance(s.ItxChain.GetContext(), receiverAcc, teststypes.UatomOsmoIbcdenom)
