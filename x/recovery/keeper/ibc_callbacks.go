...
import (
	...
	"github.com/itxnetwork/itx/v12/ibc"
...
//   - sends over all Itx native tokens
//
...
func (k Keeper) OnRecvPacket(
	...
	// Get addresses in `itx1` and the original bech32 format
	...
	// a sender's own account on Itx (sender == recipient)
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		packetTransfer := &transfertypes.MsgTransfer{
			...
			Sender:           recipient.String(),        // recipient is the address in the Itx chain
...
// the IBC denomination, i.e port and channel on Itx for the voucher. It
// returns an error if:
