...
func (suite *KeeperTestSuite) DoSetupTest(t require.TestingT) {
	...
	header := testutil.NewHeader(
		1, time.Now().UTC(), "itx_9001-1", consAddress, nil, nil,
	)
...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	s.app = suite.ItxChain.App.(*app.Itx)
	evmParams := s.app.EvmKeeper.GetParams(s.ItxChain.GetContext())
	evmParams.EvmDenom = utils.BaseDenom
	err := s.app.EvmKeeper.SetParams(s.ItxChain.GetContext(), evmParams)
	suite.Require().NoError(err)

	// s.app.FeeMarketKeeper.SetBaseFee(s.ItxChain.GetContext(), big.NewInt(1))
	...
	validators := s.app.StakingKeeper.GetValidators(suite.ItxChain.GetContext(), 2)
	cons, err := validators[0].GetConsAddr()
	suite.Require().NoError(err)
	suite.ItxChain.CurrentHeader.ProposerAddress = cons.Bytes()

	err = s.app.StakingKeeper.SetValidatorByConsAddr(suite.ItxChain.GetContext(), validators[0])
	suite.Require().NoError(err)

	_, err = s.app.EvmKeeper.GetCoinbaseAddress(suite.ItxChain.GetContext(), sdk.ConsAddress(suite.ItxChain.CurrentHeader.ProposerAddress))
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(suite.ItxChain.GetContext(), inflationtypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToAccount(suite.ItxChain.GetContext(), inflationtypes.ModuleName, suite.ItxChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(s.ItxChain.GetContext(), types.ModuleName, coins)
	s.Require().NoError(err)
	coins = sdk.NewCoins(sdk.NewCoin(teststypes.UatomIbcdenom, sdk.NewInt(100)))
	err = s.app.BankKeeper.MintCoins(s.ItxChain.GetContext(), types.ModuleName, coins)
	s.Require().NoError(err)

	// Mint coins on the osmosis side which we'll use to unlock our uitx
	...
	// Mint coins on the cosmos side which we'll use to unlock our uitx
	...
	err = s.app.ClaimsKeeper.SetParams(suite.ItxChain.GetContext(), claimparams)
	suite.Require().NoError(err)

	params := types.DefaultParams()
	params.EnableErc20 = true
	err = s.app.Erc20Keeper.SetParams(suite.ItxChain.GetContext(), params)
	suite.Require().NoError(err)
	...
	coinItx = sdk.NewCoin(utils.BaseDenom, sdk.NewInt(1000000000000000000))
	coins = sdk.NewCoins(coinItx)
	err = s.app.BankKeeper.MintCoins(suite.ItxChain.GetContext(), types.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToModule(suite.ItxChain.GetContext(), types.ModuleName, authtypes.FeeCollectorName, coins)
	suite.Require().NoError(err)
}
...
// to the Itx chain (used on IBC tests)
func (suite *KeeperTestSuite) DeployContractToChain(name, symbol string, decimals uint8) (common.Address, error) {
	return testutil.DeployContract(
		s.ItxChain.GetContext(),
		s.ItxChain.App.(*app.Itx),
		suite.ItxChain.SenderPrivKey,
		suite.queryClientEvm,
		contracts.ERC20MinterBurnerDecimalsContract,
		name, symbol, decimals,
	)
...
// Send back coins (from path endpoint B to A). In case of IBC coins need to provide ibcCoinMetadata (<port>/<channel>/<denom>, e.g.: "transfer/channel-0/uitx") as input parameter.
