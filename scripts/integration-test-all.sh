...
## default port prefixes for itxd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    ...
    "$PWD"/build/itxd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/itxd start --pruning=nothing --rpc.unsafe \
    ...
    itx_PID=$!
    echo "started itx node, pid=$itx_PID"
    # add PID to array
    arr+=("$itx_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test itx node $HOST_RPC ..."
...
stop_func() {
    itx_PID=$i
    echo "shutting down node, pid=$itx_PID ..."
